...
- **Admin application** — Next.js 16 / React 19 internal operations console: destination, place, and state content management; CSV import and export; trip moderation; KYC and identity review; and analytics dashboards using React Query, Zustand, shadcn/Radix UI, and Recharts.
- **Mobile application** — Expo 54 / React Native 0.81 consumer application with roughly 27,000 lines: Expo Router, NativeWind, Zustand and TanStack Query, Firebase authentication with Google and Apple sign-in, clustered maps, push notifications, and Sentry.
- **Purpose-specific microservice and supporting tools** — one standalone background scoring microservice for trending and popularity ranking with time-decay math, percentile normalization, and dependency-ordered score cascades across places, destinations, and states; plus supporting ETL, image-quality, and itinerary-prototyping tools rather than additional production microservices.
